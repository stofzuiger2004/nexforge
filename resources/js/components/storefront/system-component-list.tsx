...
export function SystemComponentList({ components }: SystemComponentListProps) {
    if (components.length === 0) {
        return (
            <div className="rounded-2xl border border-dashed p-10 text-center">
                <h3 className="font-semibold">No components have been added</h3>

                <p className="mt-2 text-sm text-muted-foreground">
                    Add system components in the catalogue before publishing
                    this system.
                </p>
            </div>
        );
...
function ComponentRow({ component, hasBorder }: ComponentRowProps) {
    const Icon = slotIcons[component.slot] ?? Package;

    return (
        <article className={hasBorder ? 'border-t' : undefined}>
                ...
                <div className="min-w-0">
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
                        ...
                        {component.quantity > 1 && (
                            <span>Quantity {component.quantity}</span>
                        )}
                    </div>
