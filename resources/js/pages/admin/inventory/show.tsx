...
function moneyInputValue(
    ...
    return `${Math.floor(
        amountInCents / 100,
    )}.${String(amountInCents % 100).padStart(
        2,
        '0',
    )}`;
}
...
function PriceEditor({
    ...
    price: AdminInventoryDetailPrice;
}) {
    const form = useForm({
        ...
        compare_at_amount: moneyInputValue(
            price.compare_at_amount_in_cents,
        ),
            ...
            <div className="mb-4 flex items-start justify-between gap-4">
                <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                        ...
                        {price.price_list.is_default
                            ? ' · Default'
                            : ''}
                        {!price.price_list.is_active
                            ? ' · Inactive'
                            : ''}
                    </p>
                </div>
...
export default function InventoryShow({
    ...
    const adjustmentForm = useForm({
        ...
        reference: '',
        expected_lock_version:
            inventoryItem.stock.lock_version,
        idempotency_key:
            inventoryItem.form_tokens.adjustment,
    });
    ...
    const settingsForm = useForm({
        ...
        safety_stock:
            inventoryItem.stock.safety_stock.toString(),
        reorder_point:
            inventoryItem.stock.reorder_point?.toString() ??
            '',
        is_active:
            inventoryItem.stock.is_active,
        expected_lock_version:
            inventoryItem.stock.lock_version,
    });
    ...
    return (
        ...
        >
            <Head
                title={`Inventory · ${inventoryItem.variant.sku}`}
            />
            ...
            {inventoryItem.can.view_prices &&
                ...
                <section className="mt-6 rounded-xl border bg-background p-5">
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                        Prices apply to this variant
                        across all warehouses.
                    </p>

                    <div className="mt-4 grid gap-4 xl:grid-cols-2">
                        {inventoryItem.prices.map(
                            (price) =>
                                inventoryItem.can
                                    .manage_prices ? (
                                    <PriceEditor
